...
public class DraggableCardView: UIView, UIGestureRecognizerDelegate {
	...
	@objc func panGestureRecognized(_ gestureRecognizer: UIPanGestureRecognizer) {
		...
		// Removed: scaling and rotation of card when swipped. This ensure's the y position of the current card remain immutable
		...
		case .began:
			...
			let firstTouchPoint = gestureRecognizer.location(in: self)
			let newAnchorPoint = CGPoint(x: firstTouchPoint.x / bounds.width, y: y)
			let oldPosition = CGPoint(x: bounds.size.width * layer.anchorPoint.x, y: y)
			let newPosition = CGPoint(x: bounds.size.width * newAnchorPoint.x, y: y)
			layer.anchorPoint = newAnchorPoint
			layer.position = CGPoint(x: layer.position.x - oldPosition.x + newPosition.x, y: y)
			removeAnimations()
			
			dragBegin = true
			
			animationDirectionY = y
		...
		case .changed:
			...
			transform = CATransform3DTranslate(transform, dragDistance.x, y, 0)
